...
- **Large-collection testing**: Most collection specs mock `collection:fetch` with a handful of items. To test at scale, `fixtures.ts` exposes an opt-in `largeCollection` fixture option (default `false`). A spec enables it with `test.use({ largeCollection: true })`, which launches the app with `--simulate-large-collection` so the real `SimulationService` feeds a large collection through the actual scraper/SQLite path (see `e2e/large-collection.spec.ts`). Large runs get their own `temp-test-data/<worker>-large` dir so the simulated cache never leaks into small-collection specs. First load is slow (batched, with simulated latency + 10% error rate) — use `test.setTimeout(150000)` and an `expect.poll` on the "<n> items" header (regex `/^\d[\d,]*\s+items?$/`); later tests in the same file reuse the SQLite cache and are fast.
- **Simulation size is configurable**: `SimulationService` defaults to 5000 items (used by `npm run dev:large` for scalability stress-testing) but honours the `SIMULATE_COLLECTION_SIZE` env var. The E2E fixture sets it to `1500` so the cold build is faster while staying well above the spec's `LARGE` threshold (1000). If you change either, keep `SIMULATE_COLLECTION_SIZE` in `fixtures.ts` above `LARGE` in `large-collection.spec.ts`.
- **Virtualization assertion**: `ItemsGrid` renders `slice(0, visibleCount)` (starts at 20, +20 per IntersectionObserver hit). Assert virtualization by checking `getByTestId('album-card')` count stays bounded (≤ ~60) while the header item count is in the thousands. Trip the infinite-scroll sentinel with `cards.last().scrollIntoViewIfNeeded()`.
- **Simulation playback limits**: Simulated tracks have an empty `streamUrl` (assert queue/UI state, not audio). "Play All" still clears the idle "No track playing" label because the current track is set in state even with an empty `streamUrl`.
- **Simulated album hydration**: `SimulationService` gives each simulated album a recognisable `bandcampUrl` (`https://sim.local/album/sim-<id>`), and `ScraperService.getAlbumDetails()` short-circuits to `simulationService.getAlbumDetails()` for those URLs — serving 10 synthetic tracks with no network. This lets the lazy album-hydration loop (`CollectionView.handleBulkAction` → `hydrateAlbum`) be exercised at scale in E2E. To prove hydration ran, filter to **Albums only**, bulk "Add to Queue", and assert the queue holds more entries than there were albums (each album expands to multiple tracks). Without this, simulated albums could not hydrate (the old generic `bandcampUrl` would hit the real network). `simulation.test.ts` locks the URL-scheme contract.
- **Collection filter state leaks across E2E tests**: Filters/sort persist to settings (`store.updateSettings`) and each fresh per-test app reloads them via `fetchSettings`, so a filter toggled off in a bulk-action test leaks into later tests (and across runs). Reset to defaults in `beforeEach` with `AppHelpers.resetCollectionState()` — the store's `settings.onChanged` listener applies the reset to the live view reactively (no reload needed), since `SETTINGS_CHANNELS.SET` broadcasts `ON_CHANGED`.
